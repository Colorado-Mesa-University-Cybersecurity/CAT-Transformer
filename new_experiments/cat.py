...
# optimizer = torch.optim.Adam(params=cat_model.parameters(), lr=0.00001)
...
#MYFT
myft = MyFTTransformer(n_cont=len(cont_columns),
                       cat_feat=cat_features,
                       targets_classes=target_classes,
                       get_attn=False,
                       regression_on=True).to(device_in_use)

optimizer = torch.optim.AdamW(params=myft.parameters(), lr=0.0001, weight_decay=0.00001)
loss_function = nn.MSELoss()
